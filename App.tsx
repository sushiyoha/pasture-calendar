...
const App: React.FC = () => {
  ...
  // --- 修复后的 handleToggleTask ---
  const handleToggleTask = async (taskId: string) => {
    // 1. 第一步：先在本地数据里找到这个任务
    // 我们需要知道它现在的状态（是完成还是未完成），以及它属于哪个格子
    let targetTask: Task | undefined;
    let targetGridId: string | undefined;

    // 遍历 tasksMap 寻找任务
    for (const [gridId, tasks] of Object.entries(tasksMap)) {
    ...
    // 如果没找到（比如出鬼了），直接报错返回
    if (!targetTask || !targetGridId) {
      console.error("在本地找不到任务:", taskId);
      return;
    }

    // 2. 计算新状态（取反）
    const newStatus = !targetTask.completed;
    console.log(`准备切换任务状态: ID=${taskId}, 新状态=${newStatus}`);

    // 3. 乐观更新 UI (Optimistic Update)
    // 这一步是为了让你点击时立刻有反应，不用等服务器
    setTasksMap(prev => ({
      ...prev,
      [targetGridId!]: prev[targetGridId!].map(t =>
    ...
    // 4. 真正发送请求给服务器
    try {
      // 这一步会触发 taskService 里的 console.log
      await taskService.toggle(taskId, newStatus);
      console.log("服务器同步成功！");
    } catch (error) {
      console.error("服务器同步失败，正在回滚...", error);
